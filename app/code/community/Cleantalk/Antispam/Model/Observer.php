...
{
    protected Mage_Core_Model_Abstract|null|false|Cleantalk_Antispam_Model_Client $client;
    ...
    public function interceptOutput(Varien_Event_Observer $observer)
    {
        ...
        if (strpos($html, '<div class="middle"') !== false && Mage::app()
                                                                  ->getRequest()
                                                                  ->getParam('cleantalk_message') !== null) {
            $html = str_replace('<div class="middle"',
                                '<div class="notification-global  notification-global-error"><b>CleanTalk error: ' . Mage::app()
                                                                                                                         ->getRequest()
                                                                                                                         ->getParam('cleantalk_message') . '</b></div><div class="middle"',
                                $html);
        ...
        if (strpos($html, '<div class="middle"') !== false && $show_notice == 1) {
            ...
            $html = str_replace('<div class="middle"',
                                '<div class="notification-global  notification-global-notice">' . $message . '</div><div class="middle"',
                                $html);
            $transport->setHtml($html);
        }
        if (strpos($html, '%LINK_TEXT%') !== false) {
            ...
            if (trim($api_key) == '') {
                ...
                $html = str_replace('%LINK_TEXT%',
                                    "<a target='__blank' href='http://cleantalk.org/my' >Click here to get anti-spam statistics</a>",
                                    $html);
            }
            $transport->setHtml($html);
        }
    ...
    public function interceptQuery(Varien_Event_Observer $observer)
    {
        if (strpos($_SERVER['PHP_SELF'], '/downloader/') === false) {
            ...
            if (Mage::getSingleton('admin/session')->isLoggedIn() && strpos($_SERVER['PHP_SELF'],
                                                                            'system_config') !== false) {
                ...
                if (Mage::app()->getRequest()->getParam('get_auto_key')) {
                    ...
                    /** @var Cleantalk_Antispam_Model_Api_GetApiKey $getApiKeyService */
                    $getApiKeyService = Mage::getModel('antispam/api_getApiKey');
                    $getApiKeyService->setEmail($admin_email);
                    $getApiKeyService->setWebsite($site);
                    $result = $getApiKeyService->execute();
                    if ($this->accountExists($result) && !$this->hasApiKey($result)) {
                        //add openmage / magento 1.9 message for admin user that account already exists and needs to be retrieved from the website
                        $message = "Account already exists. Please retrieve your access key from the website.";
                        ...
                        if ($result) {
                            ...
                            Mage::getSingleton('adminhtml/session')
                                ->addSuccess('Access key has been successfully retrieved')
                            ;
                            header('Location: .');
                            return false;
                ...
                if (Mage::app()->getRequest()->getPost()['groups']['cleantalk']['fields']['api_key']['value']) {
                    $new_key = Mage::app()
                                   ->getRequest()
                                   ->getPost()['groups']['cleantalk']['fields']['api_key']['value'];
            ...
            if (!Mage::getSingleton('admin/session')->isLoggedIn() && sizeof(Mage::app()
                                                                                 ->getRequest()
                                                                                 ->getPost()) > 0 && (strpos($_SERVER['PHP_SELF'],
                                                                                                             '/account/create') === false || strpos($_SERVER['REQUEST_URI'],
                                                                                                                                                    '/account/forgotpassword') === false || strpos($_SERVER['PHP_SELF'],
                                                                                                                                                                                                   '/account/login') === false || strpos($_SERVER['REQUEST_URI'],
                                                                                                                                                                                                                                         '/account/login') === false || strpos($_SERVER['REQUEST_URI'],
    ...
    */
    ...
    private function accountExists(array $result): bool
    {
        return isset($result["data"]["account_exists"]) && (bool)$result["data"]["account_exists"];
    }

    private function hasApiKey(array $result): bool
    {
        return isset($result["data"]["auth_key"]) && !empty($result["data"]["auth_key"]);
    }
        ...
        if (count($arr)) {
            foreach ($arr as $key => $value) {
                ...
                if (!is_array($value) && !is_object($value)) {
                    ...
                    if (in_array($key, $skip_params, true) && $key != 0 && $key != '' || preg_match("/^ct_checkjs/",
                                                                                                    $key)) {
                        $contact = false;
                    }
                    ...
                    $temp = Cleantalk_Antispam_Model_Observer::cleantalkGetFields($value, $message, $email, $nickname,
                                                                                  $subject, $contact, $prev_name);
    ...
    public function addCleantalkColumnToCustomerGrid(Varien_Event_Observer $observer)
    {
        ...
        if ($block->getType() == 'adminhtml/customer_grid') {
            $block->addColumnAfter('is_cleantalk_spam_user', array(
                'header' => Mage::helper('customer')->__('Is Cleantalk Spam User'),
                'index' => 'is_cleantalk_spam_user',
                'type' => 'options',
                'options' => array(
                    0 => Mage::helper('customer')->__('No'),
                    1 => Mage::helper('customer')->__('Yes'),
                ),
            ),                     'email'); // Add after the 'email' column
        }
    }

    public function addAttributeToCustomerCollection(Varien_Event_Observer $observer)
    {
